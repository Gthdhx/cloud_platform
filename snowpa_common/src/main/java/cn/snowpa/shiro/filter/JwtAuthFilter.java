package cn.snowpa.shiro.filter;

import cn.snowpa.common.CustomException;
import cn.snowpa.common.JsonConvertUtil;
...
public class JwtAuthFilter extends BasicHttpAuthenticationFilter {
    ...
    /**
     * 检测Header里面是否包含 token 字段，有就进行Token登录认证授权
     */
    @Override
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (JedisUtil.exists(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                JedisUtil.setObject(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, timeout);
                ...
                this.getSubject(request, response).login(jwtToken);
                // 最后将刷新的AccessToken存放在Response的Header中的 token 字段返回
